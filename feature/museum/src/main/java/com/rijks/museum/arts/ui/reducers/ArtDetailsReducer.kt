...
class ArtDetailsReducer @Inject constructor() : Reducer<ArtDetailsScreenEvents, ArtDetailsScreenState> {

    override fun invoke(
        event: ArtDetailsScreenEvents
    ): ArtDetailsScreenState.() -> ArtDetailsScreenState = {
        when (event) {
            is ArtDetailsScreenEvents.Loading -> copy(
                isLoading = true,
                error = null,
            )

            is ArtDetailsScreenEvents.Loaded -> copy(
                isLoading = false,
                error = null,
                content = event.model,
            )
